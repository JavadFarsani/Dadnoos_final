...
export default function BlogCards({ posts, className }: BlogCardsProps) {
  ...
  return (
    <div className={cn('grid gap-6 md:grid-cols-2 xl:grid-cols-2', className)}>
      {posts.map((post) => (
        <article key={post.id} className="flex h-full flex-col overflow-hidden rounded-3xl border border-neutral-400/25 bg-white/95 dark:border-neutral-800 dark:bg-neutral-800">
          {post.coverImageUrl && (
            <div
              className="h-60 w-full bg-neutral-200 bg-cover bg-center"
              style={{ backgroundImage: `url(${post.coverImageUrl})` }}
              aria-hidden="true"
            />
          )}
          <div className="flex h-full flex-col gap-3 p-5 pt-4">
            <div className="text-neutral-500 dark:text-neutral-400">{formatDate(post.publishedAt)}</div>
            ...
            <div className="mt-auto flex items-center justify-between pt-3 text-sm">
              <span className="text-neutral-500 dark:text-neutral-400">مطالعه حدوداً ۵ دقیقه</span>
